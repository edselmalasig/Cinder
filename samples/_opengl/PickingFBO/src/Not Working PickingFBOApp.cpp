...
void PickingFBOApp::setupGeometry()
{
    ...
    for(auto idx : objIndices){
        
    }
    
    objEdgePositions.reserve( objIndices.size());
    auto mappedPosAttrib = mBatch->getVboMesh()->mapAttrib3f(geom::Attrib::POSITION, false);
    
    for( int i = 0; i < mBatch->getVboMesh()->getNumVertices() ; i++) {
        glm::vec3 &pos = *mappedPosAttrib;
        std::cout << mBatch->getVboMesh()->getNumVertices() << " " << i << " " << pos.x << " " << pos.y << " " << pos.z << std::endl;
    ...
    std::vector<vec4> objVertexPickingColors( vertexPositions.size() );
    
    objEdgesMesh = gl::VboMesh::create( objEdgePositions.size(), GL_LINES, { layout } );
    objVerticesMesh = gl::VboMesh::create( mBatch->getVboMesh()->getNumVertices(), GL_POINTS, { layout } );
    
    objEdgesMesh->bufferAttrib( geom::POSITION, objEdgePositions.size() * sizeof( vec3 ), objEdgePositions.data() );
    objEdgesMesh->bufferAttrib( geom::COLOR, edgeColors );
    
    objVerticesMesh = gl::VboMesh::create( mBatch->getVboMesh()->getNumVertices(), GL_POINTS, { layout } );
    
    objVerticesMesh->bufferAttrib( geom::POSITION, mBatch->getVboMesh()->getNumVertices() * sizeof( vec3 ), objEdgePositions.data() );
    ...
    objVerticesBatch = gl::Batch::create( mVerticesMesh, mPickableProg, objMapping );
    objEdgesBatch = gl::Batch::create( mEdgesMesh, mPickableProg, objMapping );
...
void PickingFBOApp::loadObj( const DataSourceRef &dataSource )
{
    ...
    mBatch = gl::Batch::create( *tMesh, mPickableProg );
